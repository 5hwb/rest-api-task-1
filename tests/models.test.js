...
describe("Model Test Cases", () => {
  ...
  let spaceship1 = new Spaceship(
    1,
    "Galactic Superstar 1",
    "Qwertytron 9000",
    location1
  );
  let spaceship2 = new Spaceship(
    2,
    "Outer Space Maestro 2",
    "Qwertytron 7000",
    location1
  );
  ...
  it("spaceship1 should be able to move to location2", async () => {
    ...
    spaceship1.moveLocation(location2);
  ...
  it("spaceship1 should not be able to move to location2 again", async () => {
    expect(() => {
      spaceship1.moveLocation(location2);
    }).toThrow("Spacecraft is already at this location");
  ...
  it("spaceship2 should not be able to move to location2 as its capacity has been exceeded", async () => {
    ...
    expect(() => {
      spaceship2.moveLocation(location2);
    }).toThrow("Capacity of new location has been reached");
  ...
  it("spaceship3 should not be able to move to location3 as its status is not Operational", async () => {
    ...
    expect(() => {
      spaceship3.moveLocation(location3);
    }).toThrow("Spacecraft status is not operational");
  ...
  it("should not add an incoming spaceship if capacity of location has been exceeded", async () => {
    ...
    let anotherSpaceship = new Spaceship(
      4,
      "Old thingo2",
      "1st Gen Skycar (2030)",
      dummyLoc
    ...
    expect(locationUnderTest.addIncomingSpaceship(aSpaceship)).toBe(true);
    expect(locationUnderTest.addIncomingSpaceship(anotherSpaceship)).toBe(
      false
    );
    expect(locationUnderTest.spaceshipRegistry.size).toEqual(1);
  });
  ...
  it("should not remove undefined or null spaceships", async () => {
    ...
    expect(locationUnderTest.addIncomingSpaceship(aSpaceship)).toBe(true);
    expect(locationUnderTest.removeOutgoingSpaceship(aNullSpaceship)).toBe(
      false
    );
    expect(locationUnderTest.removeOutgoingSpaceship(aNullSpaceship2)).toBe(
      false
    );
    expect(locationUnderTest.spaceshipRegistry.size).toEqual(1);
  });
